...
AS
	SELECT f.FileUniqueId, f.ParentFileUniqueId, f.FileExtensionGuid, f.ApplicationGuid, f.StorageGuid, f.DepartmentGuid, f.[Name], f.SizeInBytes, f.Height, f.Width, f.Align, f.ExpirationRequired
		, f.CreatedTime, f.UpdatedTime, f.TemporaryGuid, f.Deleted, f.[Checksum], fe.FileExtension, fe.MimeType, (s.[Path] + CASE WHEN RIGHT(s.[Path], 1) = '\' THEN '' ELSE '\' END) AS StoragePath, d.OrganizationGuid
	FROM dbo.[File] AS f
